...
perl /home/user/data3/jimj/circRNA/tumor_ribominus/script/bin/circ_ratio.pl -c ${name}.uniq.num.bed6+ -j $junc -o ${name}.uniq.num.max.bed6+
awk -F "\t" '{if($9>$10)print $0"\t"$9"\t"$5/($5+$9);else print $0"\t"$10"\t"$5/($5+$10)}' ${name}.uniq.num.max.bed6+ |cut -f1-8,11,12 >${name}.uniq.ratio.bed6+

perl /home/user/data3/jimj/circRNA/tumor_ribominus/script/bin/calculate_psi.pl --file=${name}.scan.es.uniq.bed12+ --key=${name}.uniq.ratio.bed6+ --junc=$junc --outfile=${name}.uniq.ratio.psi.bed6+
...
#perl /mnt/share/dingwq/bin/getUnion.pl -f1 ${name1}2${name2}.uniq.juncleft.bed6 -f2 ${name1}2${name2}.uniq.juncright.bed6 -k1 3 -k2 0 -k ${name1}2${name2}.common.circ.txt -p 0,1,2,3,4,5 -o ${name1}2${name2}.common.circjunc.txt
